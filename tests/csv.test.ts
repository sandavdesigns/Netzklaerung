...
test('decodes Navision DOS/OEM 850 byte 0x81 as ü',()=>{const navision='Firmenname;Ort\r\nENOMETRIK Gesellsch. für Messdienstleistg. mbH;Düsseldorf',encoded=iconv.encode(navision,'cp850'),result=decodeCsvWithInfo(encoded);assert.equal(encoded.includes(0x81),true);assert.equal(result.encoding,'DOS/OEM 850');assert.equal(result.text,navision)});
test('repairs already stored CP850 control characters',()=>{assert.equal(repairMojibake('ENOMETRIK Gesellsch. f\u0081r Messdienstleistg. mbH'),'ENOMETRIK Gesellsch. für Messdienstleistg. mbH')});
