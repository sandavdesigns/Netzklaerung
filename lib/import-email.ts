...
function insert(input:{subject:string;senderName:string;senderAddress:string;recipients:unknown[];receivedAt:string;bodyHtml:string;bodyText:string;caseId:string|null;source:string;attachmentCount:number}){const id=randomUUID(),now=new Date().toISOString();db.prepare(`INSERT INTO emails(id,graph_id,internet_message_id,subject,sender_name,sender_address,recipients,received_at,body_html,body_text,has_attachments,case_id,source,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(id,null,'',input.subject,input.senderName,input.senderAddress,JSON.stringify(input.recipients),input.receivedAt,input.bodyHtml,input.bodyText,input.attachmentCount?1:0,input.caseId,input.source,now);return id}
export async function importEmailBuffer(buffer:Buffer,filename:string,forcedCaseId?:string){
 if(/\.msg$/i.test(filename)){
  ...
  const html=info.bodyHtml||(info.html?new TextDecoder().decode(info.html):''),text=info.body||sanitizeHtml(html,{allowedTags:[],allowedAttributes:{}}),caseId=forcedCaseId||linkCase(`${info.subject||''}\n${text}`),attachments=info.attachments||[],id=insert({subject:info.subject||filename,senderName:info.senderName||'',senderAddress:info.senderSmtpAddress||info.sentRepresentingSmtpAddress||info.senderEmail||'',recipients:info.recipients||[],receivedAt:new Date(info.messageDeliveryTime||info.clientSubmitTime||info.creationTime||Date.now()).toISOString(),bodyHtml:clean(html),bodyText:text,caseId,source:'msg-drop',attachmentCount:attachments.length});
  ...
  return{id,attachments:attachments.length,caseId};
 }
 if(!/\.eml$/i.test(filename))throw new Error('Unterstützt werden Outlook-Nachrichten als .msg oder .eml.');
 const parsed=await simpleParser(buffer),html=typeof parsed.html==='string'?clean(parsed.html):'',text=parsed.text||'',to=Array.isArray(parsed.to)?parsed.to.flatMap(item=>item.value):parsed.to?.value||[],caseId=forcedCaseId||linkCase(`${parsed.subject||''}\n${text}`),id=insert({subject:parsed.subject||filename,senderName:parsed.from?.value[0]?.name||'',senderAddress:parsed.from?.value[0]?.address||'',recipients:to,receivedAt:(parsed.date||new Date()).toISOString(),bodyHtml:html,bodyText:text,caseId,source:'eml-drop',attachmentCount:parsed.attachments.length});
 ...
 return{id,attachments:parsed.attachments.length,caseId};
}
