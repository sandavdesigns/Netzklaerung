...
CREATE INDEX IF NOT EXISTS idx_cases_status_due ON cases(status,due_date); CREATE INDEX IF NOT EXISTS idx_cases_partner ON cases(partner); CREATE INDEX IF NOT EXISTS idx_activities_case ON activities(case_id,created_at); CREATE INDEX IF NOT EXISTS idx_emails_received ON emails(received_at DESC); CREATE INDEX IF NOT EXISTS idx_edifact_type ON edifact_documents(message_type,created_at DESC);
`);
